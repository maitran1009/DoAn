...
public class ProductService {
    ...
    public boolean create(ProductInput input) {
        ...
        try {
            ...
            if (input.getId() > 0) {
                ...
                detailUpdate = productRepository.findById(input.getId()).orElse(new Product()).getProductDetails();
            }
            ...
            for (ProductDetailInput item : input.getProductDetail()) {
                ...
                detail.setSize(sizeRepository.findById(item.getSize()).orElse(null));
    ...
    public ProductOutput getProductInfo(int id) {
        return new ProductOutput().convertTo(productRepository.findById(id).orElse(null));
    }
}
